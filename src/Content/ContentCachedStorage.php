 ...
 */
class ContentCachedStorage implements ContentStorageInterface, ContentStorageCacheInterface {
  use DependencySerializationTrait;

  /**
   * The content storage to be cached.
   *
   ...
   *
   * @param \Drupal\content_sync\Content\ContentStorageInterface $storage
   *   A content storage to be cached.
   * @param \Drupal\Core\Cache\CacheBackendInterface $cache
   *   A cache backend used to store content.
   */
  public function __construct(ContentStorageInterface $storage, CacheBackendInterface $cache) {
  ...
  public function readMultiple(array $names) {
    ...
    if (!empty($cache_keys)) {
      ...
      // Cache content objects that were loaded from the storage, cache
      // missing content objects as an explicit FALSE.
      $items = [];
    ...
    // Add the content objects from the cache to the list.
    ...
    // Ensure that only existing content objects are returned, filter out
  ...
  /**
   * Finds content object names starting with a given prefix.
   *
  ...
  /**
   * Returns a cache key for a content name using the collection.
   *
   * @param string $name
   *   The content name.
   *
   * @return string
   *   The cache key for the content name.
   */
  ...
  /**
   * Returns a cache key map for an array of content names.
   *
   * @param array $names
   *   The content names.
   *
   * @return array
   *   An array of cache keys keyed by content names.
   */
